...
export function MessageBubble({ message, subagents, showTimestamp = true }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div
        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${isUser ? 'bg-secondary' : 'bg-gradient-to-br from-orange-500 to-amber-600'}`}
      >
        {isUser ? (
      ...
      <div className={`flex-1 min-w-0 space-y-2 ${isUser ? 'items-end' : ''}`}>
        ...
        {message.textContent && (
          <div
            className={`rounded-lg px-3.5 py-2.5 ${isUser ? 'bg-secondary text-foreground' : 'bg-card border border-border text-foreground'}`}
          >
