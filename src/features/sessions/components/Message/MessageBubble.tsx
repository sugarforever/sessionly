...
export function MessageBubble({ message, subagents, showTimestamp = true }: MessageBubbleProps) {
  ...
  const markdownComponents = useMemo(
    () => ({
      ...
      h3: ({ children }: { children?: React.ReactNode }) => <h3 className="text-xs font-bold mt-2 mb-1">{children}</h3>,
      ...
      table: ({ children }: { children?: React.ReactNode }) => (
        <div className="overflow-x-auto my-2"><table className="min-w-full border-collapse border border-border text-xs">{children}</table></div>
      ),
      ...
      th: ({ children }: { children?: React.ReactNode }) => <th className="border border-border px-2 py-1.5 text-left font-semibold">{children}</th>,
      td: ({ children }: { children?: React.ReactNode }) => <td className="border border-border px-2 py-1.5">{children}</td>,
    }),
    []
  )
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${isUser ? 'bg-secondary' : 'bg-gradient-to-br from-orange-500 to-amber-600'}`}>
        {isUser ? <User className="h-3.5 w-3.5 text-muted-foreground" /> : <Sparkles className="h-3.5 w-3.5 text-white" />}
      </div>
      <div className={`flex-1 min-w-0 space-y-2 ${isUser ? 'items-end' : ''}`}>
        <div className={`flex items-center gap-2 ${isUser ? 'flex-row-reverse' : ''}`}>
          ...
          {showTimestamp && formattedTime && <span className="text-[10px] text-muted-foreground/50 tabular-nums">{formattedTime}</span>}
          {message.model && !isUser && <span className="text-[10px] text-muted-foreground/50 font-mono">{message.model}</span>}
        </div>
        {message.thinkingBlocks.map((block, i) => <ThinkingBlock key={`thinking-${i}`} block={block} />)}
        {message.textContent && (
          <div className={`rounded-lg px-3.5 py-2.5 ${isUser ? 'bg-secondary text-foreground' : 'bg-card border border-border text-foreground'}`}>
            <div className="prose-sm text-xs leading-relaxed space-y-2 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
              <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>{message.textContent}</ReactMarkdown>
            </div>
          </div>
        )}
